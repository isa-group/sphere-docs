...
- **Feature evaluation is always based on the Pricing Token.**  
  `space-react-client` does not call SPACE directly to check features; instead, it uses the evaluation results stored in the Pricing-Token loaded in `TokenService`.
...
- **`SpaceProvider({ config, children })`**  
  ...
  - **Props:**  
    - `config: SpaceConfiguration`  
      - `url: string` — Base URL of the SPACE instance (for example, http://localhost:5403). If SPACE is served under a sub-path, include the prefix as part of the URL (for example, http://localhost:5403/myspace).
...
:::warning
You do not need to set `/api/v1` in the `url` field, as the client will automatically append it for API calls. WebSocket connection will be also managed by the client.
:::
