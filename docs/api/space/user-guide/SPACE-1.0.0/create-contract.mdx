...
- A valid organization-level **API key** with at least the `MANAGEMENT` scope. See [SPACE Roles](../../space-roles.md) for details.
...
```
  ...
  <TabItem value="node-client" label="💻 SPACE Node Client">
```js
const newContract = {
  userContact: {
    userId: "72ee0357-e34b-4d8f-9e0f-b710391cb298",
...
- `organizationId` is automatically assigned based on the API key used for the request.
- `groupId` can OPTIONALY be provided at contract creation to assign the user to a specific group. This is useful for group-based pricing models, where the subscription plan or add-ons depend on the group the user belongs to.
