...
:::warning Important
Once a user is deleted, their user-level **API key stops working immediately** and they are removed from any organization they are in. If they are the last user in an organization, that organization will be deleted as well (see [Delete Organization](./delete-organization.md) to better understand the cascade effects of this operation).
:::