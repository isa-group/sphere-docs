...
One of the key advantages of **SPACE** over traditional subscription management solutions is its ability to support **pooled subscriptions** —that is, a single subscription that spans across multiple services.  
...
- With SPACE, you don’t need to manage these subscriptions separately —they can all live under one unified subscription context.  
...
Here’s a simplified view of the iSubscription metamodel:

```mermaid
classDiagram
    Subscription "1" -- "1..*" Service : includes
    Service *-- "0..1" ContractedPlan
    Service *-- "0..*" ContractedAddOn
```
...
On the client side, the SPACE SDKs expect the JWT to be provided in a header named Pricing-Token. The client application should intercept incoming requests, extract the token from this header, and store it locally so it can be used to resolve feature access and enforce usage limits.

:::info
Optionally, SPACE SDKs can also stablish a direct connection with the SPACE instance to retrieve the token directly from it. Nonetheless, this approach is only recommended for applications that do not have a backend, such as static websites or mobile apps.
:::
...
- **Custom claims added by SPACE:**
  ...
  - `subscriptionContext`: : Optional object containing the subscription context used during the evaluation.
  - `pricingContext`:  Optional object containing the pricing context used during the evaluation.
...
You don’t need to handle JWT generation manually. SPACE exposes an endpoint to issue pricing tokens when requested.
:::
...
- **Client-side SDKs** only offer methods to retrieve and update locally stored token, and to evaluate feature access (e.g., rendering or hiding UI components).
...
This tool is part of ongoing research by the [ISA group](https://github.com/isa-group) in pricing-driven development and operation of SaaS. It is in an **early stage** and is not intended for production use. The ISA group does not accept responsibility for any issues or damages that may arise from its use in real-world environments
