...
On the client side, SPACE expects the JWT to be accessible, since its client SDKs depend on it to resolve feature access and enforce usage limits. Depending on the client, the strategy to deliver and update these tokens may vary. Some of them may require the token to be in the localStorage, and some others may save the current state of the JWT internally.
